...
export default function Calendrier() {
  ...
  /** Ajout lamine */
  const joursPropager = []
  const joursParticulier = []
  const [renderOneOrTwo, setRenderOneOrTwo] = React.useState(true)
  const daysComming = [0]
  const daysMissed = ['15-10-2024']
        ...
        renderOneOrTwo ? (
          <div className='calendarContainer'>
            <div className='calendarContainerWrapper'>
              <div className="buttonIteraction">
                <ButtonVP name='Ajouter' />
                <ButtonVP name='Modifier' />
                <ButtonVP name='Supprimer' />
              </div>


              <Calendar
                className={'ConfigCalendar'}
                allowPartialRange={true}

                tileClassName={({ activeStartDate, date, view }) => {
                  const formatMyDate = `${date.getDate()}-${date.getMonth()}-${date.getFullYear()}`
                  if (date.getDay() in daysComming) {
                    return 'highlightNextDays'
                  }
                  console.log(formatMyDate)
                  if (daysMissed.includes(formatMyDate)) {
                    return 'highlightMissedDays'
                  }
                }}

                tileContent={ ({ date, view }) => {
                  if (date.getDay() === 0) {
                    return <p>ronde next</p>
                  }
                }
                }

                onClickDay={() => console.log("A day is clicked")}
              />
            </div>
          </div>
        ) : (
